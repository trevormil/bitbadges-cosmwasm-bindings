 ...
 This crate provides the custom bindings that are used to communicate with the custom modules on the BitBadges network from a CosmWasm smart contract.
...
```
 ...
 All the commands from the [`NFT Module`](https://github.com/CudoVentures/bitbadges-node#the-following-commands-are-available-click-on-them-for-further-info) are available and callable from a smart contract. 
...
use bitbadges_cosmwasm::{create_mint_nft_msg};
​
...
​
...
```rust
// src/contract.rs
use bitbadges_cosmwasm::{ BitBadgesQuerier, DenomResponse, Denom};
​
...
...
bitbadges-noded tx wasm store $clonedDir/bindings_tester.wasm --from=<address> --chain-id=<chain-id> --gas=auto -y
INIT='{}'
CODE='1' 
bitbadges-noded tx wasm instantiate $CODE $INIT --from=<address> --label="tester" --chain-id=<chain-id> --gas=auto -y
TESTER=$(bitbadges-noded query wasm list-contract-by-code $CODE --output json | jq -r '.contracts[-1]')
echo $TESTER
...
bitbadges-noded tx wasm execute $TESTER $issueDenomQuery --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded tx wasm execute $TESTER $mintNft --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded tx wasm execute $TESTER $editNft --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded tx wasm execute $TESTER $transferNft --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded tx wasm execute $TESTER $transferDenom --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded tx wasm execute $TESTER $addApprovedAddress --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded tx wasm execute $TESTER $addApproveAll --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded tx wasm execute $TESTER $revokeNFT --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded tx wasm execute $TESTER $burnNft --from=<address> --chain-id=<chain-id> --gas=auto -y 
...
bitbadges-noded query wasm contract-state smart $TESTER $denomByIdQuery --output json
...
bitbadges-noded query wasm contract-state smart $TESTER $denomsQuery --output json
...
bitbadges-noded query wasm contract-state smart $TESTER $collectionQuery --output json
...
bitbadges-noded query wasm contract-state smart $TESTER $supplyQuery --output json
...
bitbadges-noded query wasm contract-state smart $TESTER $ownerQuery --output json
...
bitbadges-noded query wasm contract-state smart $TESTER $nftQuery --output json
...
bitbadges-noded query wasm contract-state smart $TESTER $approvalsQuery --output json
...
bitbadges-noded query wasm contract-state smart $TESTER $approvedForAllQuery --output json
