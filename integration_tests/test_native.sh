...
# CREATE USERS
echo "CREATE USERS"
echo yes | BitBadges_NODED keys add alice --keyring-backend test
echo yes | BitBadges_NODED keys add bob --keyring-backend test

aliceAddress=$(BitBadges_NODED keys show -a alice --keyring-backend test)
bobAddress=$(BitBadges_NODED keys show -a bob --keyring-backend test)

# ADD FUNDS
echo "ADD FUNDS"
RES=$(BitBadges_NODED tx bank send faucet "$aliceAddress" 1000000000000000000000000abitbadges --keyring-backend test --chain-id="$CHAIN_ID" $FEE_FLAGS -y)
RES=$(BitBadges_NODED tx bank send faucet "$bobAddress" 1000000000000000000000000abitbadges --keyring-backend test --chain-id="$CHAIN_ID" $FEE_FLAGS -y)

# STORE CODE
echo "STORE CODE"
RES=$(BitBadges_NODED tx wasm store "$CONTRACT_PATH" --from alice $FEE_FLAGS --keyring-backend test --chain-id=$CHAIN_ID -y)
CODE_ID=$(echo $RES | jq -r '.logs[0].events[-1].attributes[0].value')
echo "Deployed code id: $CODE_ID"

# INSTANTIATE CONTRACT
echo "INSTANTIATE CONTRACT"
RES=$(BitBadges_NODED tx wasm instantiate $CODE_ID "{}" --label "test bindings" --no-admin --from alice $FEE_FLAGS --keyring-backend test --chain-id="$CHAIN_ID" -y)
...
RES=$(BitBadges_NODED tx bank send faucet "$CONTRACT" 100000000000000000000abitbadges --keyring-backend test --chain-id="$CHAIN_ID" $FEE_FLAGS -y)
...
QUERY_RES=$(BitBadges_NODED query wasm contract-state smart $CONTRACT $QUERY_BALANCE --output json)
...
QUERY_RES=$(BitBadges_NODED query wasm contract-state smart $CONTRACT $QUERY_ALL_BALANCE --output json)
...
QUERY_RES=$(BitBadges_NODED query wasm contract-state smart $CONTRACT $QUERY_BONDED_DENOM --output json)
DENOM=$(echo $QUERY_RES | jq -r '.data.denom')

if [ $DENOM != 'abitbadges' ];then
    printf '%s\n' "Query bonded denom failed: ""$QUERY_RES"" " >&2
    exit 1
fi

# GET VALIDATOR
QUERY_RES=$(BitBadges_NODED query staking validators --output json)
...
QUERY_RES=$(BitBadges_NODED query wasm contract-state smart $CONTRACT $QUERY_ALL_DELEGATIONS --output json)
...
QUERY_RES=$(BitBadges_NODED query wasm contract-state smart $CONTRACT $QUERY_DELEGATION --output json)
...
QUERY_RES=$(BitBadges_NODED query wasm contract-state smart $CONTRACT $QUERY_ALL_VALIDATORS --output json)
...
QUERY_RES=$(BitBadges_NODED query wasm contract-state smart $CONTRACT $QUERY_VALIDATOR --output json)
...
# CREATE PROPOSAL
RES=$(BitBadges_NODED tx gov submit-proposal software-upgrade TestSoft --upgrade-height=200000 --from alice --title TestUpdate --description TestDescription --deposit 1abitbadges --keyring-backend test --chain-id="$CHAIN_ID" $FEE_FLAGS -y)
RES_CODE=$(echo $RES | jq -r '.code')
...
# ACTIVATE PROPOSAL
RES=$(BitBadges_NODED tx gov deposit $PROPOSAL_ID 50000000000000000000000abitbadges --from $aliceAddress --keyring-backend test --chain-id="$CHAIN_ID" $FEE_FLAGS -y)
RES_CODE=$(echo $RES | jq -r '.code')
...
VOTER=$(BitBadges_NODED query gov votes $PROPOSAL_ID --output json | jq -r '.votes[0].voter')
...
echo "ADD FUNDS TO CONTRACT"
RES=$(BitBadges_NODED tx bank send faucet "$INSTANTIATED_CONTRACT_ADDR" 100000000000000000000abitbadges --keyring-backend test --chain-id="$CHAIN_ID" $FEE_FLAGS -y)
